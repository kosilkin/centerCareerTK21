    ...
    const defaultState = {
      settings: {
        ...
        headerText: "Площадка для студентов и выпускников: запись на консультации, карьерные события и полезные ссылки по направлениям обучения.",
        infoTitle: "Инфо",
        infoText: "Центр карьеры помогает студентам с поиском практики, подготовкой резюме и карьерными консультациями. На сайте можно записаться на нужный формат помощи, посмотреть мероприятия и открыть таблицы по направлениям.",
        ...
        directionsTitle: "Направления и таблицы",
      ...
      directions: [
        { name: "ИСИП", sub: "Информационные системы", desc: "Ссылка на таблицу с актуальными вакансиями и практикой по направлению.", link: "#" },
        { name: "Дизайн", sub: "Графика и интерфейсы", desc: "Ссылка на таблицу с предложениями и полезными материалами по направлению.", link: "#" },
        { name: "Экономика", sub: "Аналитика и финансы", desc: "Ссылка на таблицу с вакансиями и стажировками для студентов направления.", link: "#" },
        { name: "Юриспруденция", sub: "Правовые дисциплины", desc: "Ссылка на таблицу с подборкой возможностей и полезных ссылок.", link: "#" }
      ]
    };
      ...
      directionsGrid.innerHTML = state.directions.map((item) => `
        <div class="direction-card">
          ...
          <div class="direction-body">
            ...
            <a class="direction-link" href="${safeLink(item.link)}" target="_blank" rel="noopener noreferrer">Открыть таблицу</a>
          </div>
        </div>
      `).join("");
    }
    ...
    function renderDirectionsAdminList() {
      ...
      root.innerHTML = state.directions.map((item, index) => `
        <div class="item">
          ...
          <p class="item-text">${escapeHtml(item.desc || "—")}</p>
          <p class="item-text"><b>Ссылка:</b> ${escapeHtml(item.link || "—")}</p>
          <div class="item-actions">
    ...
    async function loadStateFromDatabase() {
      ...
      if (data && data.content) {
        ...
        state = {
          ...
          directions: Array.isArray(raw.directions) ? raw.directions : cloneDefaultState().directions
        };
      } else {
        state = {
          ...
          directions: cloneDefaultState().directions
        };
      }
